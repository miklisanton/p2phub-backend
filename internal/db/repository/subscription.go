...
func (repo *SubscriptionRepository) Save(subscription *models.Subscription) error {
	...
	if subscription.Id != 0 {
		query := `UPDATE subscriptions SET user_id = $1, created_at = $2, valid_until = $3 WHERE id = $4`
		_, err := repo.db.Exec(query, subscription.User_id, subscription.Created_at, subscription.Valid_until, subscription.Id)
		if err != nil {
			return fmt.Errorf("error updating subscription : %v", err)
		}
		return nil
	} else {
		query := `INSERT INTO subscriptions (user_id, created_at, valid_until) VALUES ($1, $2, $3) RETURNING id`
		err := repo.db.QueryRow(query, subscription.User_id, subscription.Created_at, subscription.Valid_until).Scan(&subscription.Id)
		if err != nil {
