...
func (contr *Controller) CreateOrder(c echo.Context) error {
    ...
    var base64Req []byte
    base64.StdEncoding.Encode(base64Req, jsonReq)

    sign := md5.Sum(append(base64Req, []byte(os.Getenv("GATEWAY_API_KEY"))...))

    req, err := http.NewRequest("POST", "https://cryptomus.com/api/v1/invoice", nil)
    if err != nil {
        return err
    }
